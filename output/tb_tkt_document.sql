...
(
  ...
  RecordIndicator STRING OPTIONS(description="A numeric value that identifies the table in which the row resides. All rows belonging to TktDocument have a value of 18 in this column."),
  ...
  PointOfTktIssuance STRING OPTIONS(description="Where the document was issued. Pseudo City Code (PCC) or geographic city code of the ticket issuing agent. If the agent used the "AAA" entry to change their Agent Assembly Area (AAA), this attribute will contain the PCC to which the AAA was changed. Otherwise, it will equal the Employee Profile Record (EPR) Create City or Home Station. If the segment was created via teletype, this is the city code from the signature line (Communications Reference Element) of the teletype message. Length of this field will vary based on ticketing location."),
  ...
  ExchgTktAmt NUMERIC OPTIONS(description="Amount of additional monies collected when a ticket is exchanged for another ticket. Note: The amount is determined by the differences between base + taxes on the ticket(s) exchanged against the values that pricing determines is the value of the new ticket."),
  ...
  BaseFareAmt NUMERIC OPTIONS(description="Amount of the base fare (without taxes) of all segments on the ticket. Under certain circumstances such as bulk, net, award tickets, this value will be blank."),
  TotalFareCurrCode STRING OPTIONS(description="Currency Code of the Total Amount on the ticket."),
  TotalDocAmt NUMERIC OPTIONS(description="Total price of this ticket document including base fare, taxes and fees stated in the EquivBaseFareCurrCode. In the case of exchanges this amount may be zero or may only represent the difference in price from the previous ticket amount."),
  ...
  EquivBaseFareAmt NUMERIC OPTIONS(description="The base fare stated in the currency the traveler paid in which is usually the travel point of origin if different from the agency's default currency but could also reflect the point of sale currency when different from the agency's default currency."),
  ...
  FareCalcType INT64 OPTIONS(description="A unique system-assigned numeric value representing the classification of fare calculation text. 1 = IATA  2 = ATB 3 = BT – Fare calculation line for Bulk ticket 4 = IT – Fare calculation line for Inclusive Tour ticket 5 = NEW – Fare calculation line for normal fare ticket"),
  ...
  OriginalIATANbr STRING OPTIONS(description="In the case of an exchange or reissue transaction, this is the IATA number of the location where the first (original)document was issued. This IATA number is carried forward to subsequent reissued/exchanged documents."),
  ...
  RFICode STRING OPTIONS(description="Reason For Issuance Code.  Populated for document types such as compensation vouchers VOU and VCH."),
  RFITxt STRING OPTIONS(description="Reason For Issuance Text. Free text describing the reason a document was issued.  Populated primarily for document types, such as compensation vouchers, but may be present for other document types as well."),
  ...
  OtherFeeAmt NUMERIC OPTIONS(description="During an exchange transaction, amount associated with 'Other' (OTH) type fee that is charged by the carrier. Example: LTA (lost ticket application) fee."),
  ...
  OBFeePricingCode STRING  OPTIONS(description="0  OB Fee Priced Code - Indicates how fee was priced. Possible values are: - system priced - agent priced"),
  ...
  OBFeeTotal NUMERIC OPTIONS(description="Total OB Fee amount charged. Value could be 0 if fee is waived."),
  ...
  BaseFareAmtUSD NUMERIC OPTIONS(description="Amount from BaseFareAmt column converted to USD using BSR applicable to date of ticket issueance."),
  TotalDocOwnerAmt NUMERIC OPTIONS(description="Total amount on the ticket, expressed in the woning carrier’s currency."),
  TotalDocAmtUSD NUMERIC OPTIONS(description="Total price of this ticket document including base fare, taxes and fees stated in the TotalDocAmt column, converted to USD using the BSR applicable to date of ticket issueance."),
)
